        ...
        {bookingObjects.map((bookingObject) => (
          <div key={bookingObject.objId}>
            {bookingObject.extLink ? (
              <a href={bookingObject.extLink} target="_blank" rel="noopener noreferrer" className="object-titles flex items-center h-10 min-w-10 text-left text-sm p-1 m-0 pl-3 border-r-2 border-l-0 border-b-4 border-white bg-slate-100 hover:bg-slate-300  truncate">
                {bookingObject.title}
              </a>
            ) : (
